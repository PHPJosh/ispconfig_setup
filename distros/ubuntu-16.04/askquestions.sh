...
AskQuestions() {
	  ...
	  while [ "x$CFG_SQLSERVER" == "x" ]
          do
                CFG_SQLSERVER=$(whiptail --title "SQLSERVER" --backtitle "$WT_BACKTITLE" --nocancel --radiolist "Select SQL Server type" 20 75 2 "MySQL" "(default)" ON "MariaDB" "" OFF "Percona Mysql" "" OFF 3>&1 1>&2 2>&3)
          done
